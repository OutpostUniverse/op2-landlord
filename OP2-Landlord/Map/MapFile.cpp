...
{
	mCameraPosition = {std::clamp(x, 0, mCameraAnchorArea.size.x), std::clamp(y, 0, mCameraAnchorArea.size.y)};
}
...
void MapFile::load(const std::string& filename)
{
	...
	try
	{
		StreamReader stream_reader(Utility<Filesystem>::get().readFile(VirtualPath{filename}));
