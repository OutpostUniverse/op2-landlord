#include "MiniMap.h"

#include "Common.h"

#include <NAS2D/EventHandlerMouseButton.h>


using namespace NAS2D;

MiniMap::MiniMap()
{
	_init();
}


MiniMap::~MiniMap()
{}


void MiniMap::_init()
{
	position(2, 34);
	size(70, 70);
	text("Mini Map");
}


void MiniMap::adjustCamera(int x, int y)
{
	Renderer& r = Utility<Renderer>::get();
	mMap->setCamera((TILE_SIZE * (x - (int)rect().position.x + 4)) - ((int)r.size().x / 2),(TILE_SIZE * (y - (int)rect().position.y - 21)) - ((int)r.size().y / 2));
}


void MiniMap::mouseDown(EventHandler::MouseButton b, NAS2D::Point<int> position)
{
	if (b != EventHandler::MouseButton::Left) { return; }
	
	mLeftButtonDown = true;

	if (!mMiniMap) { return; }

	const auto startPoint = NAS2D::Point{rect().position.x + 4, rect().position.y + 21}.to<int>();
	const auto miniMapBounds = NAS2D::Rectangle{startPoint.x, startPoint.y, mMiniMap->size().x, mMiniMap->size().y};
	if (miniMapBounds.contains(position))
	{
		mMovingCamera = true;
		adjustCamera(position.x, position.y);
	}

}


void MiniMap::mouseUp(EventHandler::MouseButton b, NAS2D::Point<int> position)
{
	if (b != EventHandler::MouseButton::Left) { return; }

	mLeftButtonDown = false;
	mMovingCamera = false;
}


void MiniMap::mouseMotion(NAS2D::Point<int> position, NAS2D::Vector<int> change)
{
	if (dragging()) { return; }

	if(mMovingCamera)
	{
		adjustCamera(position.x, position.y);
		return;
	}
}


void MiniMap::draw()
{
	Renderer& r = Utility<Renderer>::get();

	r.drawBoxFilled(rect(), NAS2D::Color{180, 180, 180});
	r.drawBoxFilled({rect().position.x, rect().position.y, rect().size.x, 16}, NAS2D::Color{75, 95, 130});
	r.drawBox(rect(), NAS2D::Color::Black);


	r.drawBoxFilled({rect().position.x + 5, rect().position.y + 21, static_cast<float>(mMiniMap->size().x), static_cast<float>(mMiniMap->size().y)}, NAS2D::Color{255, 0, 255});
	r.drawImage(*mMiniMap, {rect().position.x + 5, rect().position.y + 21});

	mViewRect = {static_cast<int>(rect().position.x + 5 + (mMap->cameraPosition().x / TILE_SIZE)), static_cast<int>(rect().position.y + 21 + (mMap->cameraPosition().y / TILE_SIZE)), static_cast<int>(r.size().x / TILE_SIZE), static_cast<int>(r.size().y / TILE_SIZE)};
	r.drawBox(mViewRect);
}


void MiniMap::update_minimap()
{
	createMiniMap();
	size(mMiniMap->size().x + 10, mMiniMap->size().y + 26);
}


void MiniMap::createMiniMap()
{
	if (!mMap) { throw std::runtime_error("MiniMap::createMiniMap(): mMap not set!"); }

	std::vector<Color> buffer(mMap->width() * mMap->height());
	std::size_t offset = 0;
	for (int y = 0; y < mMap->height(); y++)
	{
		for (int x = 0; x < mMap->width(); x++)
		{
			buffer[offset++] = mMap->tile_color(x, y);
		}
	}

	delete mMiniMap;
	mMiniMap = new Image(buffer.data(), sizeof(Color), {mMap->width(), mMap->height()});
}
